...
'''
    ...
    my_midi = b"MThd\x00\x00\x00\x06" + \
...
'''
    ...
    while len(my_midi) >= 8:
        ...
        if track_type != b'MTrk':
            _warn('midi2opus: Warning: track #'+str(track_num) +
                  ' type is '+str(track_type)+" instead of b'MTrk'")
        [track_length] = struct.unpack('>I', my_midi[4:8])
        my_midi = my_midi[8:]
        if track_length > len(my_midi):
            _warn('midi2opus: track #'+str(track_num) +
                  ' length '+str(track_length)+' is too large')
            _clean_up_warnings()
            return my_opus   # 5.0
    ...
    for opus_track in tracks:
        ...
        for opus_event in opus_track:
            ...
            if opus_event[0] == 'note_off' or (opus_event[0] == 'note_on' and opus_event[4] == 0):
                ...
                if chapitch2note_on_events.get(key):
                    ...
                    _warn('opus2score: note_off with no note_on cha=' +
                          str(cha)+' pitch='+str(pitch))
            elif opus_event[0] == 'note_on':
                cha = opus_event[2]
                pitch = opus_event[3]
                key = cha*128 + pitch
                new_event = ['note', ticks_so_far,
                             0, cha, pitch, opus_event[4]]
                if chapitch2note_on_events.get(key):
        ...
        for chapitch in chapitch2note_on_events:
            ...
            for new_e in note_on_events:
                ...
                _warn("opus2score: note_on with no note_off cha=" +
                      str(new_e[3])+' pitch='+str(new_e[4])+'; adding note_off at end')
    ...
    while i < len(score):
        ...
        if len(new_track) > 0:
            for c in channel2patch_num:
                new_track.append(
                    ['patch_change', start_time, c, channel2patch_num[c]])
        ...
        for channel in channels_so_far & new_channels:
            ...
            while itrack < len(input_score):
                for input_event in input_score[itrack]:
                    channel_index = Event2channelindex.get(
                        input_event[0], False)
...
'''
    ...
    while i < len(opus_or_score):
        ...
        for event in opus_or_score[i]:
            if event[0] == 'note':
                num_notes_by_channel[event[3]] = num_notes_by_channel.get(
                    event[3], 0) + 1
                if event[3] == 9:
            ...
            # 4.8
                ...
                is_a_score = False
                num_notes_by_channel[event[2]] = num_notes_by_channel.get(
                    event[2], 0) + 1
                if event[2] == 9:
...
'''
    ...
    while integer > 0:
        seven_bits = 0x7F & integer
        # XXX surely should convert to a char ?
...
def _clean_up_warnings():  # 5.4
    ...
    if _previous_times > 1:
        print('  previous message repeated ' +
              str(_previous_times)+' times', file=sys.stderr)
    ...
    while (len(trackdata)):
        ...
        if (first_byte < 0xF0):  # It's a MIDI event
            ...
            if (command == 0x80):
                ...
                if 'key_after_touch' in exclude:
                    continue
                E = ['key_after_touch', time, channel,
                     parameter[0], parameter[1]]
            elif (command == 0xB0):
                if 'control_change' in exclude:
                    continue
                E = ['control_change', time, channel,
                     parameter[0], parameter[1]]
            elif (command == 0xC0):
            ...
            # Now the sticky events -------------------------------------
                ...
                E = ['smpte_offset', time] + \
                    list(struct.unpack(">BBBBB", trackdata[0:5]))
            elif (command == 0x58):
        ...
        ######################################################################
            ...
            if first_byte == 0xF0:
                ...
                E = ['sysex_f0', time,
                     trackdata[0:length].decode('ISO-8859-1')]
            else:
                E = ['sysex_f7', time,
                     trackdata[0:length].decode('ISO-8859-1')]
            trackdata = trackdata[length:]
        ...
        if (   # MIDI events -- eligible for running status
            ...
            # End of text meta-events
                ...
                event_data = struct.pack(
                    ">BBBbBBBB", 0xFF, 0x54, 0x05, E[0], E[1], E[2], E[3], E[4])
            elif (event == 'time_signature'):
                # event_data = struct.pack(">BBwBBBB",  0xFF, 0x58, 4, E[0:4] )
                event_data = struct.pack(
                    ">BBBbBBB", 0xFF, 0x58, 0x04, E[0], E[1], E[2], E[3])
            elif (event == 'key_signature'):
                event_data = struct.pack(
                    ">BBBbB", 0xFF, 0x59, 0x02, E[0], E[1])
            ...
            # Other Things...
                ...
                event_data = bytearray(
                    b'\xF0')+_ber_compressed_int(len(E[0]))+bytearray(bytes(E[0], encoding='ISO-8859-1'))
            elif (event == 'sysex_f7'):
                #event_data = struct.pack(">Bwa*", 0xF7, len(E[0]), E[0])
                event_data = bytearray(
                    b'\xF7')+_ber_compressed_int(len(E[0]))+bytearray(bytes(E[0], encoding='ISO-8859-1'))
